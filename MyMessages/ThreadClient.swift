...
let POLLING_INTERVAL = 1.0 // seconds
...
class ThreadClient {
    ...
    init(threadId: String) {
        ...
        let baseUrl = URL(string: RESOURCE_URL)!

        let authPolicy = try! CommunicationUserCredentialPolicy(
            credential: CommunicationUserCredential(token: UserDirectory.currentUserToken())
        )
        let options = AzureCommunicationChatClientOptions(
            dispatchQueue: DispatchQueue.main
        )
        chatClient = try! AzureCommunicationChatClient(endpoint: baseUrl, authPolicy: authPolicy, withOptions: options)
    ...
    public func onMessage(_ completionHandler: @escaping (Message) -> Void) {
        ...
        self.timer = Timer.scheduledTimer(withTimeInterval: POLLING_INTERVAL, repeats: true) { (timer) in
